import { Guild, User, TextChannel, Message } from "discord.js";
...
export default class WhitelistService {
  static async start(server: Guild, user: User) {
    ...
    if (CacheService.has(cacheKey) === false) {
      const textChannel: TextChannel | undefined = await ChannelService.create(
        server,
        user
      );
  ...
  static async complete(textChannel: TextChannel, user: User) {
    ...
    if (approved) {
      await textChannel.members
        .get(user.id)
        ?.roles?.add(process.env.WHITELIST_ROLE_ID as string);
    } else {
      const cacheKey = "@whitelist/failed/" + user.id;
