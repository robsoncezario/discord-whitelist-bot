...
export default class MessageService {
	static addCommands(message: Message): void {
		try {
			if (message.content.startsWith(process.env.COMMAND_PREFIX as string)) {
				...
				if (args.length >= 1) {
					const rawCommand = args.shift()!.toLowerCase().slice((process.env.COMMAND_PREFIX as string).length)
	...
	static async filter(message: Message) {
		try {
			if (message.channel.id == process.env.WHITELIST_CHANNEL_ID && message.author.bot === false) {
