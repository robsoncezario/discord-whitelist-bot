...
export default class ChannelService {
  static async create(
    ...
    try {
      ...
      const exists: GuildChannel | ThreadChannel | undefined =
        server.channels.cache.find((c) => c.name == nameOfChannel);
  ...
  static async prepare(client: Client) {
    const channel = client!.channels!.cache!.get(
      process.env.WHITELIST_CHANNEL_ID as string
    ) as TextChannel;
    var message = null;
    ...
    if (channel) {
      try {
        message = await channel.messages.fetch(
          process.env.WHITELIST_MESSAGE_ID as string
        );
        message.pin();
      } catch {
